...
class SetorPotonganController extends CI_Controller
{
    ...
    {
        $data['page_title'] = 'SETOR POTONGAN BOK';
        $this->template->set('title', 'SETOR POTONGAN BOK');
        $this->template->load('template', 'bok/setorpotongan/index', $data);
    }
    ...
    public function no_terima()
    {
        ...
        $sql = "SELECT a.* FROM bok_trhtrmpot a
    ...
    public function loaddata_trmpot()
    {
        ...
        $sql = "SELECT a.* FROM bok_trdtrmpot a
    ...
    public function update_potongan()
    {
        ...
        $query3 = $this->db->query("UPDATE bok_trdtrmpot set ntpn='$ntpn',ebilling='$ebilling' WHERE no_bukti='$no_terima' and kd_skpd='$skpd' and kd_rek6='$kd_rek'");
        echo '1';
    }
    ...
    public function simpan_data()
    {
        ...
        $sql = "INSERT into bok_trdstrpot $lckolompot values $nilaipot";
        $asg = $this->db->query($sql);
        if ($asg) {
            $this->db->insert('bok_trhstrpot', array(
            ...
            $this->db->query("UPDATE bok_trhtrmpot set [status]='1' WHERE no_bukti='$no_terima' and kd_skpd='$cskpd'");
            echo '1';
        } else {
            echo '0';
        }
    ...
    function loaddata()
    {
        ...
        $sql = "SELECT count(*) as total from bok_trhstrpot where kd_skpd='$kd_skpd' $where ";
        ...
        $sql = "SELECT top $rows no_bukti,tgl_bukti,no_terima,kd_skpd,no_sp2d,RTRIM(jns_spp) as jns_spp,npwp,ket,nilai
				from bok_trhstrpot where kd_skpd='$kd_skpd' $where AND no_bukti not in (SELECT top $offset no_bukti FROM bok_trhstrpot where 
    ...
    public function update_data()
    {
        ...
        $ctgl = $this->input->post('ctgl');
        $cskpd  = $this->input->post('cskpd');
        $data = $this->db->query("UPDATE bok_trhstrpot set ket='$cket', tgl_bukti=' $ctgl' WHERE no_bukti='$cno' and kd_skpd='$cskpd'");
        if ($data) {
            echo '1';
        } else {
            echo '0';
    ...
    public function hapus_data()
    {
        ...
        $this->db->query("DELETE FROM bok_trdstrpot WHERE kd_skpd='$skpd' AND no_bukti='$nomor'");
        $this->db->query("DELETE FROM bok_trhstrpot WHERE kd_skpd='$skpd' AND no_bukti='$nomor'");
        $data = $this->db->query("UPDATE bok_trhtrmpot set [status]='0' WHERE no_bukti='$no_terima' and kd_skpd='$skpd'");
        if ($data) {
            echo '1';
        } else
            echo '0';
