...
class ctrmpot extends CI_Controller {
	...
	function load_trans(){
        ...
        */
       $sql = "select * from(
                    SELECT DISTINCT a.no_tgl,a.no_voucher,a.tgl_voucher,b.no_sp2d,b.kd_sub_kegiatan,b.nm_sub_kegiatan,b.kd_rek6,b.nm_rek6,a.jns_spp,a.total 
                    FROM trhtransout_cmsbank a
                    JOIN trdtransout_cmsbank b ON a.no_voucher = b.no_voucher and a.kd_skpd = b.kd_skpd
                    WHERE a.kd_skpd = '$kode' and a.no_voucher not in (select no_voucher from trhtrmpot_cmsbank a where a.kd_skpd = '$kode') 
                    and a.status_upload not in ('1') and a.jns_spp in ('1','2','3')
                ) as a union
                select '' no_tgl,no_panjar [no_voucher],tgl_panjar [tgl_voucher],'' no_sp2d,kd_sub_kegiatan,'' [nm_sub_kegiatan],'' [kd_rek6],'' [nm_rek6],'1' [jns_spp],
                nilai [total]
	...
	function load_kegiatan_pot(){
        ...
        $query1 = $this->db->query("select DISTINCT * from(            
                                    SELECT DISTINCT a.kd_sub_kegiatan,a.nm_sub_kegiatan FROM trdtransout_cmsbank a
                                                INNER JOIN trhtransout_cmsbank c ON a.no_voucher = c.no_voucher
                                                AND a.kd_skpd = c.kd_skpd
                                                WHERE a.no_sp2d = '$sp2d'  
                                    union 
                                    select a.kd_sub_kegiatan,b.nm_sub_kegiatan from tr_panjar_cmsbank a join trdrka b on a.kd_sub_kegiatan=b.kd_sub_kegiatan where a.kd_skpd='$skpd'  and a.no_panjar='$no_panjar'
                                    ) as c  ");              
        $result = array();
        $ii = 0;
        foreach($query1->result_array() as $resulte){ 
         ...
         if ($tabel == 'trhtrmpot_cmsbank') {
            ...
            // Simpan Detail //                       
				...
				if (!($asg)){
                    ...
                    $sql = "insert into trdtrmpot_cmsbank(no_bukti,kd_rek6,nm_rek6,nilai,kd_skpd,kd_rek_trans,ebilling,rekanan,npwp)"; 
         ...
         if ($tabel == 'trhtrmpot_cmsbank') {
            ...
            // Simpan Detail //                       
				...
				if (!($asg)){
                    ...
                    $sql = "insert into trdtrmpot_cmsbank(no_bukti,kd_rek6,nm_rek6,nilai,kd_skpd,kd_rek_trans,ebilling)"; 
                    $asg = $this->db->query($sql.$csql);
					if (!($asg)){
                       $msg = array('pesan'=>'0');
                        echo json_encode($msg);
	...
	function hapus_trmpot(){
		...
		$sql 		= "update trhtransout_cmsbank set status_trmpot=0 where no_voucher='$no_voucher' AND kd_skpd='$kd_skpd'";
        $asg 		= $this->db->query($sql);
