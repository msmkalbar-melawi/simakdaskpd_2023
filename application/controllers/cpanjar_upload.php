...
class cpanjar_upload extends CI_Controller {
    ...
    function load_list_belumpanjar_upload_cms(){
        ...
        $query1 = $this->db->query("SELECT a.*,b.nm_sub_kegiatan,isnull((select sum(nilai) from trhtrmpot_cmsbank where no_voucher=a.no_panjar and kd_skpd=a.kd_skpd ),0) [pot],
            isnull((select sum(nilai) from tr_panjar_transfercms where no_bukti=a.no_panjar and kd_skpd=a.kd_skpd),0) [bersih] 
            FROM tr_panjar_cmsbank a  join trskpd b on a.kd_sub_kegiatan=b.kd_sub_kegiatan AND a.kd_skpd=b.kd_skpd 
        where a.kd_skpd='$skpd 'AND b.jns_ang='$jns_ang' AND a.status_upload='0' $and         
...
a.bank_tujuan,a.ket_tujuan,b.kd_kegiatan,b.nm_kegiatan,c.bersih     
    ...
    function load_list_telahupload(){
        ...
        $sql = "SELECT b.no_upload,count(*) as total from tr_panjar_cmsbank a left join trdupload_cmsbank_panjar b on b.kd_skpd=a.kd_skpd and a.no_panjar=b.no_bukti 
        where $init_skpd and a.status_upload='1' and a.status_validasi='0' $and group by b.no_upload
        " ;
        $query1 = $this->db->query($sql);
        $total = $query1->row();
...
FROM
    tr_panjar_cmsbank a
LEFT JOIN trdupload_cmsbank_panjar b ON b.kd_skpd = a.kd_skpd
AND a.no_panjar = b.no_bukti
WHERE
    $init_skpd
AND a.status_upload = '1'
...
GROUP BY
    ...
    b.no_upload_tgl
ORDER BY
    CAST (b.no_upload AS INT),
    CAST (a.no_panjar AS INT),
    a.kd_skpd");        
        $result = array();
        $ii     = 0;
        foreach($query1->result_array() as $resulte)
    ...
    function csv_cmsbank_panjar($nomor=''){
        ...
        $sqlquery = $this->db->query("SELECT * FROM (SELECT a.tgl_upload,a.kd_skpd,(SELECT obskpd from ms_skpd where kd_skpd=a.kd_skpd) as nm_skpd,
        ...
        where a.kd_skpd='$skpd' and a.no_upload='$nomor') x
    ...
    if($tabel == 'trdupload_cmsbank_panjar') {
            // Simpan Detail //               
                $sql_h = "select count(*) as jum from trdupload_cmsbank_panjar where no_upload='$nomor_up' AND kd_skpd='$skpd'";
                    $asg_h = $this->db->query($sql_h)->row();
                    $inith = $asg_h->jum; 
                    
                    if($inith>1){
                        $sql = "delete from trdupload_cmsbank_panjar where no_bukti='$nomor' and no_upload='$nomor_up' AND kd_skpd='$skpd'";
                        $asg = $this->db->query($sql);
                        ...
                        where left(trhupload_cmsbank_panjar.kd_skpd,7)=left('$skpd',7)
                        ";
                        $asg = $this->db->query($sql);                        
                        ...
                        $sql = "delete from trhupload_cmsbank_panjar where no_upload='$nomor_up' AND kd_skpd='$skpd'";
                        $asg = $this->db->query($sql);                                   
                    }                        
                ...
                if (!($asg)){
                    ...
                    $sql = "update tr_panjar_cmsbank set status_upload='0', tgl_upload='' where no_kas='$nomor' AND kd_skpd='$skpd'";
                    $asg = $this->db->query($sql);                    
