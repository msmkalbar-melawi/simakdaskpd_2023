...
class Tukd2 extends CI_Controller {
	...
	function load_rek_koreksi() {                      
        ...
        $sql = "SELECT a.no_bukti, a.kd_rek5, a.nm_rek5,nilai FROM trdtransout a INNER JOIN trhtransout b ON a.no_bukti=b.no_bukti AND a.kd_skpd=b.kd_skpd 
	...
	function register_pajak2($lcskpd='',$nbulan='',$ctk='',$ttd1='',$tgl_ctk='',$ttd2='', $jns='',$spasi=''){
        ...
        $sqlttd1="SELECT nama as nm,nip as nip,jabatan as jab, pangkat FROM ms_ttd where kd_skpd='$lcskpd' and kode='BK' and nip='$ttd1'";
			...
			$cRet ='<TABLE width="100%" style="font-size:16px">
					...
					<tr></tr>
                    <TR>
						<TD align="center" ><b>REGISTER PAJAK LAINNYA </TD>
					</TR>
					</TABLE><br/>';
			...
			$cRet .='<TABLE style="border-collapse:collapse;font-size:14px" border="1" cellspacing="'.$spasi.'" cellpadding="'.$spasi.'" width="100%" >
					 ...
					 <TR>
                        ...
                        <TD bgcolor="#CCCCCC" rowspan="2" align="center">Tanggal</TD>
						<TD bgcolor="#CCCCCC" rowspan="2" align="center">Uraian</TD>						
						<TD bgcolor="#CCCCCC" colspan="3" align="center">Potongan Lainnya</TD>
						<TD bgcolor="#CCCCCC" rowspan="2" align="center">Pemotongan</TD>
						<TD bgcolor="#CCCCCC" rowspan="2" align="center">Penyetoran</TD>
						<TD bgcolor="#CCCCCC" rowspan="2" align="center">Jumlah</TD>
					 </TR>
					 <TR>
					 <TD bgcolor="#CCCCCC" align="center">PPNPN</TD>
				...
				$query = $this->db->query("SELECT * FROM(
						SELECT 
						a.no_bukti,tgl_bukti, ket
						,CASE WHEN b.kd_rek5='2110901' THEN b.nilai ELSE 0 END AS ppnpn
						,CASE WHEN b.kd_rek5='2110701' THEN b.nilai ELSE 0 END AS iwp
						,CASE WHEN b.kd_rek5='2110501' THEN b.nilai ELSE 0 END AS taperum,
						b.nilai as terima,
						0 as setor
						FROM trhtrmpot a INNER JOIN trdtrmpot b ON a.no_bukti=b.no_bukti AND a.kd_skpd=b.kd_skpd WHERE a.kd_skpd='$lcskpd' 
						AND MONTH(a.tgl_bukti)='$nbulan'  AND b.kd_rek5 IN ('2110901','2110701','2110501')
						UNION ALL
						SELECT 
						a.no_bukti,tgl_bukti, ket
						,0 AS ppnpn
						...
						b.nilai as setor
						FROM trhstrpot a INNER JOIN trdstrpot b ON a.no_bukti=b.no_bukti AND a.kd_skpd=b.kd_skpd WHERE a.kd_skpd='$lcskpd' 
						AND MONTH(a.tgl_bukti)='$nbulan' AND b.kd_rek5 IN ('2110901','2110701','2110501')) a
						ORDER BY CAST(a.no_bukti as int)");  
				...
				foreach ($query->result() as $row) {
					...
					$cRet .='<TR>
                                ...
                                <TD align="left" >'.$this->tukd_model->tanggal_ind($tanggal).'</TD>
								<TD align="left" >'.$ket.'</TD>								
								<TD align="right" >'.number_format($ppnpn,"2",",",".").'</TD>
				...
				$cRet .='<TR>
								<TD colspan="3" align="right" >Jumlah bulan '.$this->tukd_model->getBulan($nbulan).'</TD>
								<TD align="right" >'.number_format($ppnpn_t,"2",",",".").'</TD>
			...
			$query = $this->db->query("	SELECT 
					SUM(ppnpn) as ppnpn_l
					,SUM(iwp) as iwp_l
					,SUM(taperum) as taperum_l
					,SUM(terima) as terima_l
					,SUM(setor) as setor_l FROM(
					SELECT 
					SUM(CASE WHEN b.kd_rek5='2110901' THEN b.nilai ELSE 0 END) AS ppnpn
					,SUM(CASE WHEN b.kd_rek5='2110701' THEN b.nilai ELSE 0 END) AS iwp
					,SUM(CASE WHEN b.kd_rek5='2110501' THEN b.nilai ELSE 0 END) AS taperum
					,SUM(b.nilai) as terima,
					0 as setor
					FROM trhtrmpot a INNER JOIN trdtrmpot b ON a.no_bukti=b.no_bukti AND a.kd_skpd=b.kd_skpd WHERE a.kd_skpd='$lcskpd' 
					AND MONTH(a.tgl_bukti)<='$nbulan_l'  AND b.kd_rek5 IN ('2110901','2110701','2110501')
					UNION ALL
					SELECT 
					0 AS ppnpn
					,0 AS iwp
					...
					SUM(b.nilai) as setor
					FROM trhstrpot a INNER JOIN trdstrpot b ON a.no_bukti=b.no_bukti AND a.kd_skpd=b.kd_skpd WHERE a.kd_skpd='$lcskpd'
					AND MONTH(a.tgl_bukti)<='$nbulan_l'   AND b.kd_rek5 IN ('2110901','2110701','2110501')) a
					 "); 
			foreach ($query->result() as $row) {	
				$ppnpn_l =$row->ppnpn_l;
				$iwp_l =$row->iwp_l;
				...
				$cRet .='<TR>
						<TD colspan="3" align="right" >Jumlah sampai bulan Sebelumnya</TD>
						<TD align="right" >'.number_format($ppnpn_l,"2",",",".").'</TD>
				...
				$cRet .='<TR>
						<TD colspan="3" align="right" >Jumlah sampai bulan '.$this->tukd_model->getBulan($nbulan).'</TD>
						<TD align="right" >'.number_format($ppnpn_l+$ppnpn_t,"2",",",".").'</TD>
        ...
        $sqlttd1="SELECT nama as nm,nip as nip,jabatan as jab, pangkat FROM ms_ttd where kd_skpd='$lcskpd' and kode='BK' and nip='$ttd1'";
			...
			$cRet .='<TABLE style="border-collapse:collapse;font-size:14px" border="1" cellspacing="'.$spasi.'" cellpadding="'.$spasi.'" width="100%" >
					 ...
					 <TR>
						...
						<TD bgcolor="#CCCCCC" colspan="9" align="center">CP</TD>
	...
	function cetak_spj_periode($lcskpd='',$tgl1='',$tgl2='',$ttd1='',$tgl_ctk='',$ttd2='',$ctk='',$atas='', $bawah='', $kiri='', $kanan='', $jenis=''){
        ...
        $cRet .="
           ...
           <tr>
            ...
            $csql = "SELECT 
					...
					SUM(ISNULL(jlain_brjs_ll,0)) jlain_brjs_ll, SUM(ISNULL(jlain_brjs_ini,0)) jlain_brjs_ini
					 FROM(
					SELECT 
					SUM(CASE WHEN b.jns_spp IN ('1','2','3') AND (b.tgl_bukti)<'$tgl1' AND a.kd_rek5 NOT IN ('2130101','2130201','2130401','2130301','2110901','2110701','2110501','2130501') THEN  ISNULL(a.nilai,0) ELSE 0 END) AS jlain_up_ll,
					SUM(CASE WHEN b.jns_spp IN ('1','2','3') AND (b.tgl_bukti) BETWEEN '$tgl1' AND '$tgl2'  AND a.kd_rek5 NOT IN ('2130101','2130201','2130401','2130301','2110901','2110701','2110501','2130501') THEN  ISNULL(a.nilai,0) ELSE 0 END) AS jlain_up_ini,
					SUM(CASE WHEN b.jns_spp IN ('4') AND (b.tgl_bukti)<'$tgl1' AND a.kd_rek5 NOT IN ('2130101','2130201','2130401','2130301','2110901','2110701','2110501','2130501') THEN  ISNULL(a.nilai,0) ELSE 0 END) AS jlain_gaji_ll,
					SUM(CASE WHEN b.jns_spp IN ('4') AND (b.tgl_bukti) BETWEEN '$tgl1' AND '$tgl2'  AND a.kd_rek5 NOT IN ('2130101','2130201','2130401','2130301','2110901','2110701','2110501','2130501') THEN  ISNULL(a.nilai,0) ELSE 0 END) AS jlain_gaji_ini,
					SUM(CASE WHEN b.jns_spp IN ('6') AND (b.tgl_bukti)<'$tgl1' AND a.kd_rek5 NOT IN ('2130101','2130201','2130401','2130301','2110901','2110701','2110501','2130501') THEN  ISNULL(a.nilai,0) ELSE 0 END) AS jlain_brjs_ll,
					SUM(CASE WHEN b.jns_spp IN ('6') AND (b.tgl_bukti) BETWEEN '$tgl1' AND '$tgl2'  AND a.kd_rek5 NOT IN ('2130101','2130201','2130401','2130301','2110901','2110701','2110501','2130501') THEN  ISNULL(a.nilai,0) ELSE 0 END) AS jlain_brjs_ini
					FROM trdtrmpot a INNER JOIN trhtrmpot b ON a.no_bukti=b.no_bukti AND a.kd_skpd=b.kd_skpd
          ...
          $csql = "SELECT 
            ...
            $tox_awal="SELECT isnull(sld_awal,0) AS jumlah FROM ms_skpd where kd_skpd='$lcskpd' and '1'= MONTH($tgl1)";
            $hasil = $this->db->query($tox_awal);
