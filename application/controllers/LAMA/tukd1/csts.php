   ...
   function validasi_sts_bp(){
		...
		$num = $prvn->nomor;
			...
			$sql3 = "insert into trdkasin_ppkd
						...
						AND LEFT(b.kd_skpd,7) IN ('4.02.03') 
						AND b.kd_skpd NOT IN ('4.02.03.01','4.02.02.02')
						AND a.status=1) x 
			...
			$sql4 = "insert into trhkasin_ppkd(no_kas,tgl_kas,no_sts,kd_skpd,tgl_sts,keterangan,total,kd_kegiatan,jns_trans,sumber) 
						...
						SELECT a.*,(select nm_skpd from ms_skpd where kd_skpd=a.kd_skpd) nm_skpd 
										...
										WHERE jns_trans IN ('4','2') AND (a.tgl_sts BETWEEN '$tglvalid1' and '$tglvalid2') 
											  AND LEFT(kd_skpd,7) IN ('4.02.03') 
											  AND kd_skpd NOT IN ('4.02.03.01','4.02.02.02')
											  AND status=1
											  AND no_sts+kd_skpd not in (select no_sts+kd_skpd from trhkasin_ppkd WHERE jns_trans IN ('4','2') AND (a.tgl_sts BETWEEN '$tglvalid1' and '$tglvalid2') AND LEFT(kd_skpd,7) IN ('4.02.03') 
											  AND kd_skpd NOT IN ('4.02.03.01','4.02.02.02')))x WHERE kd_skpd = '$skpd'";
			...
			$sql5 = "update b set b.no_kas=a.no_kas
								...
								where LEFT(b.kd_rek5,1)=4 AND b.kd_skpd = '$skpd' AND (a.tgl_sts BETWEEN '$tglvalid1' and '$tglvalid2') AND a.jns_trans in ('4','2')";
