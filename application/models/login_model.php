...
{
$CI =& get_instance();
$user_id  = $this->session->userdata('pcUser');
$query = $this->db->query("UPDATE [user] SET status = '0' WHERE id_user = '$user_id'");
...
{	
	...
	//$query = $this->db->query("SELECT * FROM [user] WHERE user_name = '$username' AND status ='0'");
	$cek = $query->num_rows();
	if($cek != 0)
	{
