<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    ...
    function rka_rinci_penyempurnaan($skpd,$kegiatan,$rekening,$norka) {
        ...
        { 
            ...
            $result[] = array(
                        ...
                        'volume'    => doubleval($resulte['volume_sempurna']),
                        'koefisien' => $resulte['koefisien_sempurna'],

                        'volume1' => doubleval($resulte['volume_sempurna1']),  
                        'volume2' => doubleval($resulte['volume_sempurna2']),  
                        'volume3' => doubleval($resulte['volume_sempurna3']),
                        'volume4' => doubleval($resulte['volume_sempurna4']),

                        'satuan' => $resulte['satuan_sempurna'],  
                        'satuan1' => $resulte['satuan_sempurna1'],  
                        'satuan2' => $resulte['satuan_sempurna2'],  
                        'satuan3' => $resulte['satuan_sempurna3'],
                        'satuan4' => $resulte['satuan_sempurna4'],  
                        'harga1'  => number_format($resulte['harga_sempurna1'],"2",".",","),  
                        'total'   => number_format($resulte['total_sempurna1'],"2",".",",")
                        );
                        $ii++;
        }
    ...
    function rka_rinci_penyempurnaan2($skpd,$kegiatan,$rekening,$norka) {
    	...
    	function get_penjabaran_apbd($n2,$whare1,$whare2,$whare3,$whare4,$whare5,$whare6,$whare1_a,$whare2_a,$whare3_a,$whare4_a,$whare5_a,$whare6_a){
            $hasil = '';
            $sql1="SELECT a.kd_rek1 AS kd_rek,rtrim(a.kelompok) AS rek, a.nm_rek1 AS nm_rek ,SUM(b.nilai) AS nilai,
                SUM(CASE WHEN $whare1 $whare1_a THEN b.$n2 ELSE 0 END) AS nilai2
                FROM ms_rek1 a INNER JOIN trdrka b ON a.kd_rek1=LEFT(b.kd_rek6,(LEN(a.kd_rek1))) 
                where $whare1 $whare1_a GROUP BY a.kd_rek1,a.kelompok, a.nm_rek1
                UNION ALL 
                SELECT a.kd_rek2 AS kd_rek,rtrim(a.kelompok) AS rek,a.nm_rek2 AS nm_rek ,SUM(b.nilai) AS nilai,
                SUM(CASE WHEN $whare2 $whare2_a THEN b.$n2 ELSE 0 END) AS nilai2
                FROM ms_rek2 a INNER JOIN trdrka b ON a.kd_rek2=LEFT(b.kd_rek6,(LEN(a.kd_rek2))) 
                where $whare2 $whare2_a GROUP BY a.kd_rek2,a.kelompok,a.nm_rek2 
                UNION ALL 
                SELECT a.kd_rek3 AS kd_rek,rtrim(a.kelompok) AS rek,a.nm_rek3 AS nm_rek ,SUM(b.nilai) AS nilai,
                SUM(CASE WHEN $whare3 $whare3_a THEN b.$n2 ELSE 0 END) AS nilai2
                FROM ms_rek3 a INNER JOIN trdrka b ON a.kd_rek3=LEFT(b.kd_rek6,(LEN(a.kd_rek3))) 
                where $whare3 $whare3_a GROUP BY a.kd_rek3,a.kelompok, a.nm_rek3
                UNION ALL 
                SELECT a.kd_rek4 AS kd_rek,rtrim(a.kelompok) AS rek,a.nm_rek4 AS nm_rek ,SUM(b.nilai) AS nilai,
                SUM(CASE WHEN $whare4 $whare4_a THEN b.$n2 ELSE 0 END) AS nilai2
                FROM ms_rek4 a INNER JOIN trdrka b ON a.kd_rek4=LEFT(b.kd_rek6,(LEN(a.kd_rek4))) 
                where $whare4 $whare4_a GROUP BY a.kd_rek4,a.kelompok, a.nm_rek4
                UNION ALL 
                SELECT a.kd_rek5 AS kd_rek,rtrim(a.kelompok) AS rek,a.nm_rek5 AS nm_rek ,SUM(b.nilai) AS nilai,
                SUM(CASE WHEN $whare5 $whare5_a THEN b.$n2 ELSE 0 END) AS nilai2 
                FROM ms_rek5 a INNER JOIN trdrka b ON a.kd_rek5=LEFT(b.kd_rek6,(LEN(a.kd_rek5))) 
                where $whare5 $whare5_a GROUP BY a.kd_rek5,a.kelompok, a.nm_rek5 
                 UNION ALL 
                SELECT a.kd_rek6 AS kd_rek,rtrim(a.kelompok) AS rek,a.nm_rek6 AS nm_rek ,SUM(b.nilai) AS nilai,
                SUM(CASE WHEN $whare6 $whare6_a THEN b.$n2 ELSE 0 END) AS nilai2 
                FROM ms_rek6 a INNER JOIN trdrka b ON a.kd_rek6=LEFT(b.kd_rek6,(LEN(a.kd_rek6))) 
                where $whare6 $whare6_a GROUP BY a.kd_rek6,a.kelompok, a.nm_rek6 ORDER BY kd_rek ";
          ...
          function terbilang_get_valid($str,$from,$to,$min=1,$max=9){
            ...
            function terbilang($nominal){
                ...
                for ($i=0; $i < strlen($decimal) ; $i++) { 
                    $buf.=" ".$angka[(int)substr($decimal, $i,1)];
                }
        ...
        function qcek_anggaran_kas($skpd,$angg){
            ...
            $csql ="SELECT * from(
    					select a.kd_skpd,a.kd_sub_kegiatan,a.ang,isnull(b.kas,0) kas from(
    						select kd_skpd,kd_sub_kegiatan,kd_rek6,sum($angg) [ang] from trdrka where kd_skpd='$skpd' and left(kd_rek6,1)='5'
    						group by kd_skpd,kd_sub_kegiatan,kd_rek6
    					)as a left join (
    						select kd_skpd,kd_sub_kegiatan,kd_rek6,sum(nilai_ubah) [kas] from trdskpd_ro where kd_skpd='$skpd' and left(kd_rek6,1)='5'
    						group by kd_skpd,kd_sub_kegiatan,kd_rek6
    					)b on b.kd_skpd=a.kd_skpd and b.kd_sub_kegiatan=a.kd_sub_kegiatan  and b.kd_rek6=a.kd_rek6
